...
#ifndef SPEED_VECTOR_HPP
#define SPEED_VECTOR_HPP
...
struct SpeedVector
   ...
   */
  constexpr
  SpeedVector(Angle bearing, double norm)
    : bearing(bearing), 
      norm(norm)
    {
    }
  ...
  SpeedVector(double x, double y)
    : bearing(Angle::FromXY(y,x).AsBearing()),
      norm(sqrt(x * x + y * y)) 
      {
      }
  ...
  static constexpr SpeedVector Zero()
    {
    return SpeedVector(Angle::Zero(), double(0));
    }
  ...
  SpeedVector Reciprocal() const
    {
    return SpeedVector(this->bearing.Reciprocal(), this->norm);
    }
  };

static_assert(std::is_trivial<SpeedVector>::value, "type is not trivial");

#endif  // SPEED_VECTOR_HPP
